...
var UpdatePassport = createReactClass({
    ...
    showImage: function(e){
        this.setState({
            // imagesave: URL.createObjectURL(e.target.files[0])
    ...
    uploadDocument: function(){
        ...
        axios.post('https://akachi-task-manager-api.herokuapp.com/updatePassport', data).then((res)=>{
            
           
           if(res.data === "good"){
            
            localStorage.setItem("passport1", "good")
            alert("Passport succesfully uploaded")
            this.props.history.push('/vaccount/upload-document')
           }

           else{
               localStorage.setItem("passport1","bad")
               alert("passport could not be uploaded")
           }
            ...
            <div>
                <div className="container8">
                    ...
                    <p style={{color: "whitesmoke"}}><FaArrowLeft id="back" onClick={this.back} style={{marginLeft:"10%", marginRight: "27%", height:"25px",width:"5%"}}/>UPDATE</p>
                      <p className="Dattention">Upload your photograph</p>
                      <form>
                      ...
                      <input type="file" id="upload" onChange={this.showImage} name="Passport" style={{display: "none"}}/>
