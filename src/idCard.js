...
var IDCard = createReactClass({
    ...
    uploadDocument: function(){
        ...
        if(document.querySelector("#upload").value != ""){
            localStorage.setItem('idCard',"good")
         
            const data = new FormData() 
            data.append('File',this.state.imagesave)
 
        axios.post('https://akachi-task-manager-api.herokuapp.com/idCard', data).then((res)=>{
            
           
           localStorage.setItem("IdCard", res.data)
            ...
            console.log(err)
                ...
                localStorage.setItem('idCard',"bad")
            }
